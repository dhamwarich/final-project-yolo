"""
Module: Visualizer BBox V4 Normalized (Accurate Color Display)
Visualization with actual shirt and pants colors from BGR means.
"""
...
class BBoxVisualizerV4Norm:
    """Visualizer with accurate shirt and pants color patches."""
    ...
    def draw_track_bbox_normalized(
        self,
        frame: np.ndarray,
        bbox: Tuple[float, float, float, float],
        track_id: int,
        shirt_color: np.ndarray,
        pants_color: np.ndarray,
        conf: float
    ) -> np.ndarray:
        """
        Draw bounding box with track ID and actual shirt/pants colors.
        
        Args:
            frame: Input frame
            bbox: Bounding box (x1, y1, x2, y2)
            track_id: Track ID
            shirt_color: Shirt BGR color (from masked mean)
            pants_color: Pants BGR color (from masked mean)
        ...
        # Convert numpy arrays to tuples for OpenCV
        shirt_bgr = tuple(map(int, shirt_color))
        pants_bgr = tuple(map(int, pants_color))
...
def draw_normalized_overlay(
    ...
    """
    Draw normalized tracking visualization with actual shirt/pants colors.
    
    Args:
        frame: Input frame
        tracks: List of track dictionaries
        bboxes: Dictionary mapping track IDs to bounding boxes
        visualizer: BBoxVisualizerV4Norm instance
    ...
    for track in tracks:
        ...
        # Use average colors for stable visualization
        shirt_color = track.get('avg_shirt_color', np.array([128, 128, 128]))
        pants_color = track.get('avg_pants_color', np.array([128, 128, 128]))
        
        annotated = visualizer.draw_track_bbox_normalized(
            annotated,
            bbox,
            track_id,
            shirt_color,

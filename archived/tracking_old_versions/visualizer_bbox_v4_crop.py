"""
Module: Visualizer BBox V4 Cropped (Vivid Color Patches)
Visualization with vivid color patches using HSV-to-BGR conversion.
"""
...
class BBoxVisualizerV4Crop:
    """Visualizer with vivid HSV-based color patches."""
    ...
    def draw_track_bbox_colorized(
        ...
        """
        Draw bounding box with vivid color patches from HSV.
        
        Args:
            frame: Input frame
            bbox: Bounding box (x1, y1, x2, y2)
            track_id: Track ID
            shirt_HSV: Shirt (H, S, V) tuple
            pants_HSV: Pants (H, S, V) tuple
        ...
        # Convert HSV to vivid BGR (high S, V for visibility)
...
def draw_colorized_overlay(
    ...
    """
    Draw colorized tracking visualization with vivid HSV-based patches.
    
    Args:
        frame: Input frame
        tracks: List of track dictionaries
        bboxes: Dictionary mapping track IDs to bounding boxes
        visualizer: BBoxVisualizerV4Crop instance
    ...
    for track in tracks:
        ...
        # Use average HSV for stable visualization
        shirt_HSV = track.get('avg_shirt_HSV', (0, 0, 0))
        pants_HSV = track.get('avg_pants_HSV', (0, 0, 0))
        
        annotated = visualizer.draw_track_bbox_colorized(
            annotated,
            bbox,
            track_id,
            shirt_HSV,

"""
Module: Re-ID Tracker V4 Stable (With Temporal Color Smoothing)
Tracking with bbox smoothing and temporal color smoothing to reduce flicker.
"""
...
from tracking.appearance_extractor_v3_stable import (
...
class TrackV4Stable:
    """Track with temporal color smoothing and bbox stabilization."""
    ...
    def __init__(self, track_id: int, detection: Dict):
        ...
        # Temporal color smoothing (rolling buffer of last 5 frames)
        ...
        # HSV accumulation for long-term average
    ...
    def update(self, detection: Dict, alpha: float = 0.2):
        ...
        # Update HSV values (add to buffers for temporal smoothing)
    ...
    def get_state(self) -> Dict:
        ...
        return {
            ...
            'shirt_HSV': self.shirt_HSV,  # Smoothed HSV
            'pants_HSV': self.pants_HSV,  # Smoothed HSV
...
class ReIDTrackerV4Stable:
    """Re-ID tracker with temporal color smoothing and bbox stabilization."""
    ...
    def __init__(
        ...
        self.tracks: List[TrackV4Stable] = []
        self.next_id = 0
    ...
    def update(self, detections: List[Dict]) -> List[Dict]:
        ...
        for det_idx in unmatched_dets:
            ...
            if memory_id is not None:
                new_track = TrackV4Stable(memory_id, detections[det_idx])
        ...
        for det_idx in remaining_dets:
            new_track = TrackV4Stable(self.next_id, detections[det_idx])
            self.tracks.append(new_track)
            self.next_id += 1
    ...
    def get_all_track_HSV_with_variance(self) -> Dict[int, Dict]:
        """Get average HSV and variance for all active tracks."""
        hsv_dict = {}
